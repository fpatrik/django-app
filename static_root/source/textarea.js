...
function Textarea(form, step, page, parameters){
	
	//Set the variable name of the textarea, throw exception if not found
	this.variable_name = parameters.variable_name || console.error('Variable name of Textarea input not set!');
	...
	this.add_alert({'condition' : 
		function(self, end, index){
			if(self.page.type == 'page'){
				if(self.required() && end && (typeof variables[self.step.variable_name][self.page.variable_name][self.variable_name] == 'undefined' || variables[self.step.variable_name][self.page.variable_name][self.variable_name] == '')){
					return true;
				}
			}
			else if(self.page.type = 'multipage'){
				if(self.required() && end && (typeof variables[self.step.variable_name][self.page.variable_name][index][self.variable_name] == 'undefined' || variables[self.step.variable_name][self.page.variable_name][index][self.variable_name] == '')){
					return true;
				}
			}
	...
	//METHOD renders the textarea input
	this.render = function(index){
		//Get current value if present, otherwise use default if present
		this.value = get_value(this, index);

		//render
		return nunjucks.renderString(template_textarea , {textarea : this, navigation : navigation, index : index});
	}
}