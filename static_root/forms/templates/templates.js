...
template_form = "\
<div class='row'>\
    <nav class='col-sm-3' id='Stepmenu'>\
...
<li class='{% if loop.index0 == navigation.step %}active{% endif %}'>\
	<a class='scrollmenu' onclick=\"navigation['step']={{step.step_id}};navigation['page']=zero_or_next_page({{step.step_id}}).page;navigation['multipage']['overview']=true;window.form.render();\">{{ step.title(step) }}{% if step.has_alert %}<span class='glyphicon glyphicon-exclamation-sign' style='color:rgb(169, 68, 66);margin-left:5px;'></span>{% elif navigation.end %}<span class='glyphicon glyphicon-ok' style='color:rgb(92,184,92);margin-left:5px;'></span>{% endif %}</a>\
...
<li class='next'><a onmouseup=\"navigation['step']=get_next_page({{ page.step.step_id }}, {{ page.page_id }}).step;navigation['page']={{ page.next_page().page }};navigation['multipage']['overview']=true;window.form.render();\">Weiter</a></li>\
{% if page.form.debug %}\
<li class='next'><a onmouseup=\"alert(JSON.stringify(window.variables));console.log(JSON.stringify(window.variables));\">Eingaben Anzeigen</a></li>\
{% endif %}\
<li class='next'><a onclick=\"document.getElementById('submit_input').value=JSON.stringify(window.variables);document.getElementById('django_csrf').value=django_csrf;document.getElementById('submit_data').action=return_url;document.getElementById('submit_data').submit(); \">Speichern und Formular schliessen</a></li>\
<form action='' id='submit_data' method='POST'>\
<input type='hidden' name='csrfmiddlewaretoken' value='' id='django_csrf'>\
<input type='hidden' name='variables' value='' id='submit_input'/>\
</form>\
{% else %}\
...
//TEXT LABEL TEMPLATE
template_textlabel = "{% if textlabel.show(textlabel, index) %}\
<div class='form-group'>\
	{% if textlabel.label(textlabel, index) != '' %}<label>{{ textlabel.label(textlabel, index) }}</label>{% endif %}\
	{% if textlabel.tooltip(textlabel, index) != '' %}<a class='tooltips' data-toggle='tooltip' data-placement='right' title='{{ textlabel.tooltip(textlabel, index) }}'><i class='glyphicon glyphicon-info-sign'></i></a> {% endif %}\
	<input type='text' value='{{ textlabel.value(textlabel, index) }}' class='form-control' disabled='disabled'/>\
</div>\
{% endif %}";
...
<div class='form-group'>\
	...
	<select class='form-control' onchange='this.options[this.selectedIndex].click()' {% if dropdown.disabled(dropdown, index) %}disabled='disabled'{% endif %}>\
...
template_multipage_instance = "{% if multipage.show(multipage, index) %}\
	...
	<ul class='pager'>\
	<li class='previous'><a onmouseup=\"navigation['multipage']['overview']=true;window.form.render();\">Zurück zur Übersicht</a></li>\
	</ul>{% endif %}";

//MULTIPAGE DROPDOWN TEMPLATE
template_model_dropdown = "<div class='form-group'>\
	<select class='form-control' onchange='this.options[this.selectedIndex].click()'>\
		<option></option>\
...
template_label = "{% if label.show(label, index) %}\
	<label style='font-size:{{ label.font_size(label, index) }}px;margin-bottom:15px;margin-top:15px;display:block;'>{{ label.label(label, index) }}</label>\
{% endif %}";

//API SELECT TEMPLATE
template_apiselect = "{% if api.show(api, index) %}\
<div class='panel panel-info'>\
  <div class='panel-heading'>{{ api.title(api, index)}}</div>\
  <div class='panel-body'>\
	<div class='form-group'>\
	{% if api.has_alert(api, navigation.end, index) %}<div class='alert alert-danger'>{% endif %}\
	{% if api.label(api, index) != '' %}<label>{{ api.label(api, index) }}</label>{% endif %}\
    {% if api.tooltip(api, index) != '' %}<a class='tooltips' data-toggle='tooltip' data-placement='right' title='{{ api.tooltip(api, index) }}'><i class='glyphicon glyphicon-info-sign'></i></a> {% endif %}\
	<input type='text' id='api' value='{{ form_api.query }}' onkeyup='if(event.keyCode == 13){window.api.query=document.getElementById(\"api\").value;window.api.get_options(window.api.query);}' class='form-control'>\
	<input class='btn btn-primary' type='button' value='Suchen' onclick='window.api.query=document.getElementById(\"api\").value;window.api.get_options(window.api.query);' style='margin-top:10px;'>\
	{% if api.has_alert(api, navigation.end, index) %}\
	<ul id='alert-list'>\
	{% for alert in api.alerts %}\
	{% if alert.condition(api, navigation.end, index) %}\
		<li>{{ alert.message(api) }}</li>\
	{% endif %}\
	{% endfor %}\
	</ul>\
	</div>\
	{% endif %}\
	<hr class='on-search' style='display:none;'>\
	<p class='on-search' style='display:none;'>Bitte warten...</p>\
	{% if form_api.options.length >= 0 %}\
	<hr>\
	<table class='table table-hover'>\
	<thead>\
	  <tr>\
	...
	<tbody>\
	{% for option in form_api.options %}\
	  <tr>\
	    <td><p>{{ option[0] }}</p></td>\
	    {% if option[2] == form_api.chose_only %}\
	    <td><button type='button' onclick=\"window.api.get_selection('{{ option[1] }}');\" class='btn btn-info'><span class='glyphicon glyphicon-ok' aria-hidden='false'></span></button></td>\
	    {% else %}\
	    <td>{% if form_api.chose_only == 3 %}Keine AG{% endif %}</td>\
	    {% endif %}\
	  </tr>\
	{% endfor %}\
	</tbody>\
	...
	{% endif %}\
	{% if form_api.message.message != '' %}\
	<div class='alert alert-{{ form_api.message.type }}'><p>{{ form_api.message.message }}</p></div>\
	{% endif %}\
  </div>\
</div>\
</div>\
