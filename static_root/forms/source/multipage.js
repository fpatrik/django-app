...
* PARAMETERS
*
* title: 		 The title of the multipage, can be a string or a javascrip function returning a string
* show:			 A javascript function returning true/false, determining if page should be shown, true is default
* variable_name: The key of this multipage in the step dictionary
* tooltip:		 The optional tooltip displayed in the title
* naming:		 A function taking self and index that can be used for naming instances of multipages
* 
 */

function Multipage(form, step, parameters){
	...
	//Set show, default is function returning true
	this.naming = parameters.naming || function(self, index){return 'Not Named ' + parseInt(index);};
	
	//Set model from where suggestions are taken
	this.suggestions_model = parameters.suggestions_model || false;
	this.suggestions_active = parameters.suggestions_active || function(self){return false;};
	...
	create_page_variable(this.step.variable_name, this.variable_name, 'multipage');

	//Initialise
	this.elements = [];
	
	//Checks if any instances of the multipage have an alert
	this.any_alert = function(){
		for(var i = 0; i < this.has_alert.length; i++){
			if(this.has_alert[i]){
				return true;
			}
		}
	...
	//The number of multipage instances
	this.n_instances = variables[this.step.variable_name][this.variable_name].length;
	
	//Initialise alert flag for the page
	this.has_alert = [];
	for(var i = 0; i < this.n_instances; i++){
		this.has_alert.push(false);
	}
	...
	//METHOD adds a textlabel input to the page
	this.add_textlabel = function(parameters){
		var new_textlabel = new TextLabel(this.form, this.step, this, parameters);
		this.elements.push(new_textlabel);
		return new_textlabel;
	};
	...
	this.next_page = function(){
		if(this.page_id == this.step.n_pages - 1){
			if(this.step.step_id == this.form.n_steps - 1){
				...
				if(this.form.steps[this.step.step_id + 1].show(this.form.steps[this.step.step_id + 1]) && this.form.steps[this.step.step_id + 1].pages[0].show(this.form.steps[this.step.step_id + 1].pages[0])){
			...
			if(this.form.steps[this.step.step_id].show(this.form.steps[this.step.step_id]) && this.form.steps[this.step.step_id].pages[this.page_id + 1].show(this.form.steps[this.step.step_id].pages[this.page_id + 1])){
	...
	this.previous_page = function(){
		if(this.page_id == 0){
			if(this.step.step_id == 0){
				...
				//Check if next step is shown, if yes, return, if not, get next page of next page.
				if(this.form.steps[this.step.step_id - 1].show(this.form.steps[this.step.step_id - 1]) && this.form.steps[this.step.step_id - 1].pages[this.form.steps[this.step.step_id - 1].n_pages - 1].show(this.form.steps[this.step.step_id - 1].pages[this.form.steps[this.step.step_id - 1].n_pages - 1])){
	...
	//Add instance
	this.add_instance = function(){
		this.n_instances++;
		variables[this.step.variable_name][this.variable_name].push({})
		this.has_alert.push(false);
		this.form.render();
	};
	
	//Remove instance
	this.remove_instance = function(index){
		this.n_instances--;
		variables[this.step.variable_name][this.variable_name].splice(index, 1)
		this.has_alert.splice(index, 1)
		this.form.render();
	};
	
	//Render the page
	this.render = function(){
		if(navigation.multipage.overview){
			return nunjucks.renderString(template_multipage_overview , {multipage : this, navigation : navigation});
		}
		else{
			return nunjucks.renderString(template_multipage_instance , {multipage : this, instance : navigation.multipage.instance});
		}
	};
