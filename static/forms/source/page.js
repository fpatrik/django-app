...
function Page(form, step, parameters){
	...
	//METHOD adds an API select to the page
	this.add_apiselect = function(parameters){
		var new_api = new ApiSelect(this.form, this.step, this, parameters);
		this.elements.push(new_api);
		return new_api;
	};
	...
	this.previous_page = function(){
		if(this.page_id == 0){
			if(this.step.step_id == 0){
				//This is the first page in the first step, return false
				return {'value' : false};
			}
			else{
				//Check if previous step is shown, if yes, return, if not, get previous page of previous page.
